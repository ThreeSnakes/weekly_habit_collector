...
const classifyTweetContent = (tweets) => {
  return tweets.reduce((map, tweet) => {
    ...
    if (tags.indexOf("공부")) map["공부"].push(tweet);
    else if (tags.indexOf("개발")) map["개발"].push(tweet);
    else if (tags.indexOf("독서")) map["독서"].push(tweet);
    else if (tags.indexOf("운동")) map["운동"].push(tweet);
    else if (tags.indexOf("블로그")) map["블로그"].push(tweet);
