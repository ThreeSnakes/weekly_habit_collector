...
const classifyTweetContentByCreatedAt = (tweets) => _.chain(tweets).cloneDeep().sort((tweet) => tweet.created_at).reverse().groupBy((tweet) => moment(new Date(tweet.created_at)).utcOffset(9).format('YYYY-MM-DD')).value();
